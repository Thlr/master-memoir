...
Batkube\footnote{\url{https://www.github.com/oar-team/batkube}}, which is an adaptive interface
to Batsim for Kubernetes schedulers.

In order to build this interfaca, we re-implemented an API following Kubernetes
specifications and intercepted the scheduler's time to synchronize it with the
simulation time. This allows us to simulate lengthy workloads in a short period
using a scheduler otherwise supposed to rely on ``real'' machine time. We first
describe some technical concepts about Kubernetes and Batsim, and then describe
how we re-implemented the API, intercepted the time, and handled the
synchronization of the different times between Batsim and the scheduler.
...
\captionsource{Exchanges between Batsim and the scheduler.}{\url{https://batsim.readthedocs.io/en/latest/protocol.html}}
...
we do not support parallel jobs. Finally, Batsim proposes an energy model, but
it is not under the scope of this study.
...
requests\footnote{\url{https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/}}
...
schedulers\footnote{\url{https://kubernetes.io/docs/tasks/extend-kubernetes/configure-multiple-schedulers/}},
...
in the section \ref{sec:batmsg-list} of the appendix. More information on
Batsim's protocol is available on Batsim
documentation\footnote{\url{https://batsim.readthedocs.io/en/latest/protocol.html}},
...
the
documentation\footnote{\url{https://batsim.readthedocs.io/en/latest/output-jobs.html}}.
...
\begin{itemize}
	...
	\item The \textit{mean\_waiting\_time}, which is defined by the
		duration between the submission time and the starting time
		(here the starting time is equivalent to the time at which the
		job was scheduled. We will see later that these two times do
	not correspond in Kubernetes.)
\end{itemize}
...
\begin{figure}[h]
	...
	\captionsource{Node overview}{\url{https://kubernetes.io/docs/tutorials/kubernetes-basics/explore/explore-intro/}}
	\label{fig:node-overview}
\end{figure}

The basic processing unit of Kubernetes is called a \textit{Pod} which is
...
are bundled together in \textit{Nodes} (figure \ref{fig:node-overview}) which
...
access the underlying services. A set of nodes is called a \textit{cluster}
...
encapsulating the already encapsulated services.  In each node runs one \textit{kubelet}, which is a process responsible for communicating
with the rest of Kubernetes. More precisely, the kubelet communicates with the
\texttt{kube-api-server} which is responsible for the whole cluster. We refer
to this API as the api-server, as it is called within the Kubernetes community.
This API server, as well as the other components of the \textit{Control Plane}
...
often called the \textit{master} node and typically does not run any other
container.
...
interface \texttt{kubectl}, scheduling operations or management of cluster data
on \texttt{etcd}. Kubernetes schedulers can communicate with the api-server in
...
clients\footnote{\url{https://kubernetes.io/docs/reference/using-api/client-libraries/}}
to facilitate communication with the API. 

The most notable Kubernetes scheduler is the
\texttt{kube-scheduler}\footnote{\url{https://github.com/kubernetes/kube-scheduler}}
...
Volcano\footnote{\url{https://github.com/volcano-sh/volcano}} already used in some
...
\footnote{\url{https://github.com/kubernetes-sigs/poseidon}} which is rather an
adaptive layer to Firmament \cite{199390}. All those schedulers make use of the Go client to
...
\begin{figure}[h]
	...
	\captionsource{Components of Kubernetes}{\url{https://kubernetes.io/docs/concepts/overview/components/}}
...
documented, Kubernetes team did not write extensively about how resources are
...
which is written in Go.  The central component is the \texttt{broker}. It
...
between the two parties.  \texttt{batsky-go} intercepts the calls to Go
\texttt{time} library to ensure the scheduler's time is based on the simulation
...
scheduler time is explained in section \ref{sec:time-hijack}. The \textit{rest
...
generate its code from the api-server specification.  \texttt{translate} is a
...
OpenAPI\footnote{\url{https://www.openapis.org/about}} 2 specification which is a
...
reposotory\footnote{\url{https://github.com/kubernetes/kubernetes/blob/release-1.18/api/openapi-spec/swagger.json}},
...
go-swagger\footnote{\url{https://github.com/go-swagger/go-swagger/tree/master/examples/stream-server}}
...
OAR\footnote{\url{https://github.com/oar-team/oar3}} scheduler which was isolated and
adapted to Batsim using a script written in
python\footnote{\url{https://github.com/oar-team/oar3/blob/master/oar/kao/bataar.py}}.
In order to ensure synchronization of the time between the scheduler and
Batsim, a library called batsky\footnote{\url{https://github.com/oar-team/batsky}}
was developed in the C programming language. Once compiled with this library,
all calls from the scheduler to get machine time are redirected to the
simulator.

Unfortunately, we were not able to recompile Go using a custom C library, which
would have redirected every call to machine time to Batkube. Instead, we
...
\texttt{batsky-go}\footnote{\url{https://github.com/oar-team/batsky-go}}, which re
...
threaded code easily thanks to its \textit{go routines}. These routines allow
...
\begin{figure}[]
	\centering
	\includesvg[scale=0.8]{imgs/requester-broker}
...
Batkube\footnote{\url{https://www.github.com/oar-team/batsky-go-installer}}.
...
ensures that the internals of the scheduler function correctly. During that
